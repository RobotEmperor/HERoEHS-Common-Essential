...
void BaseModule::parse_init_offset_pose_data_(const std::string &path, const std::string &data)
{
	...
	base_module_state->mov_time_state = 4.0;

	YAML::Node tar_pose_node;
	if(data.compare("init_offset_pose") == 0)
	{
		tar_pose_node = doc["offset"];// YAML 에 string "tar_pose"을 읽어온다.
	}
	if(data.compare("init_offset_pose_zero") == 0)
	{
	...
	for (YAML::iterator it = tar_pose_node.begin(); it != tar_pose_node.end(); ++it) //tar_pose_node 벡터의 처음과 시작 끝까지 for 문으로 반복한다.
	{
		...
		// 한 줄에서 int 와 double 을 분리한다.
		value = it->second.as<double>();
		base_module_state->joint_ini_pose_goal.coeffRef(joint_name_to_id_[joint_name], 0) = value; // YAML에 로드된 초기 포즈 값을 라디안으로 바꾸고, eigen matrix 에 id 개수만큼 열을 생성한다.
	}
}
void BaseModule::initPoseMsgCallback(const std_msgs::String::ConstPtr& msg) // GUI 에서 init pose topic을 sub 받아 실
{
	...
	{
		init_pose_path = ros::package::getPath("heroehs_base_module") + "/data/init_pose.yaml";// 로스 패키지에서 YAML파일의 경로를 읽어온다.
		ROS_INFO("FILE LOAD  ::  init_pose");
		parse_init_pose_data_(init_pose_path); // YAML 파일 로드
	}
	else if(msg->data.compare("init_offset_pose") == 0 || msg->data.compare("init_offset_pose_zero") == 0)
	{
		init_pose_path = ros::package::getPath("heroehs_manager") + "/config/offset.yaml";// 로스 패키지에서 YAML파일의 경로를 읽어온다.
		parse_init_offset_pose_data_(init_pose_path, msg->data); // YAML 파일 로드
		ROS_INFO("FILE LOAD  ::  %s", msg->data.c_str());
	}
	else
		return;
	new_count_ = 1;
		...
		std::map<std::string, double> sensors)
{
	...
	if(new_count_ == 1)
	{
				...
				state_iter != dxls.end(); state_iter++)
		{
			...
			if(gazebo_check == true)
				base_module_state->joint_ini_pose_state.coeffRef(joint_name_to_id_[joint_name], 0) = result_[joint_name]->present_position_; // 가제보 상 초기위치 0
			else
			{
				if(joint_name_to_id_[joint_name] == 17)
	...
	else
	{
		...
		for(int id=1 ; id<11 ; id++)
		{
			...
			if(id == 1 || id == 3 || id == 5 || id == 4 || id == 6 || id == 8 || id == 9 || id == 10) // 방향 반대인 다이나믹셀
			{
				result_[joint_id_to_name_[id]]->goal_position_ = - motion_trajectory[id]->fifth_order_traj_gen(-base_module_state->joint_ini_pose_state(id,0),
						base_module_state->joint_ini_pose_goal(id,0),0,0,0,0,0,base_module_state->mov_time_state);
				if(gazebo_check == true)
